...
const AllProductsPage = () => {
  ...
  const [editForm, setEditForm] = useState({ name: '', category: '', price: '', status: 'Active' });
  ...
  const handleStatusChange = async (product, newStatus) => {
    try {
      ...
      const response = await api.put(`/api/products/${product._id || product.id}`, {
        status: newStatus,
      ...
      if (response && (response.success || response.data)) {
        ...
        setProducts(prevProducts =>
          prevProducts.map(p =>
            (p._id === product._id || p.id === product._id || p._id === product.id || p.id === product.id)
              ? updatedProduct
              : p
          )
        );
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      setError(null);
      try {
        // For superadmin, fetch all products without filtering
  ...
  const handleSearch = (query, filters) => {
    ...
    if (filters.status) {
      results = results.filter(product => 
        product.status && product.status.toLowerCase() === filters.status.toLowerCase()
      );
    }
    
    if (filters.minPrice) {
      results = results.filter(product => {
        const price = product.variants && product.variants[0] && typeof product.variants[0].price !== 'undefined' ? product.variants[0].price : product.price;
  ...
  const handleSaveEdit = (updatedProduct) => {
    ...
    try {
      setProducts((prev) =>
        prev.map((p) =>
          (p.id === updatedProduct.id || p._id === updatedProduct._id || p.id === updatedProduct._id || p._id === updatedProduct.id)
            ? productWithCategory
            : p
        )
      );
  ...
  return (
    <div>
      <PageHeader 
        title="All Products (Super Admin)"
      ...
      <div className="card">
        {isLoading ? (
          ...
          <EmptyState
            ...
            description="No products found. They will appear here once shops add products."
                ...
                {paginatedProducts.map((product) => (
                  <tr key={product.id || product._id} className="hover:bg-gray-50">
                      ...
                      <div className="flex items-center">
                        <div className="h-10 w-10 rounded-md bg-gray-200 flex items-center justify-center text-gray-500">
                          {product.baseImage && product.baseImage.length > 0 ? (
                            <img 
                              ...
                              alt={typeof product.name === 'object' ? (product.name?.en || product.name?.ar || 'Product') : (product.name || 'Product')}
                        ...
                        <div className="ml-3">
                          <p className="text-sm font-medium text-gray-900">
                            {typeof product.name === 'object' ? (product.name?.en || product.name?.ar || 'N/A') : (product.name || 'N/A')}
                          </p>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-4 text-sm text-gray-500">
                      {typeof product.shop === 'object' && product.shop !== null
                    ...
                    <td className="px-4 py-4 text-sm text-center">
                      {product.variants && product.variants[0] && typeof product.variants[0].stock === 'number' ? (
                        product.variants[0].stock === 0 ? (
                          ...
                          <span className="text-orange-500">{product.variants[0].stock}</span>
                        ) : (
                          <span className="text-gray-700">{product.variants[0].stock}</span>
                        )
                      ) : (
                    ...
                    <td className="px-4 py-4 text-sm">
                      <span className={`badge ${
                        ...
                        product.status === 'Hidden' ? 'badge-info' :
                        product.status === 'Draft' ? 'badge-info' :
                        // fallback to stock-based logic if status is not set
                        (!product.status && product.variants && product.variants[0] && typeof product.variants[0].stock === 'number' && product.variants[0].stock === 0) ? 'badge-error' :
                        (!product.status && product.variants && product.variants[0] && typeof product.variants[0].stock === 'number' && product.variants[0].stock <= 10) ? 'badge-warning' :
                        (!product.status ? 'badge-success' : 'badge-info')
                      }`}>
                        {product.status ? product.status : (
                          product.variants && product.variants[0] && typeof product.variants[0].stock === 'number' && product.variants[0].stock === 0 ? 'Out of Stock' :
                          product.variants && product.variants[0] && typeof product.variants[0].stock === 'number' && product.variants[0].stock <= 10 ? 'Low Stock' :
                          'Active'
                        )}
                      </span>
                    </td>
                    <td className="px-4 py-4 text-center text-sm font-medium">
                      <select
                        ...
                        onChange={(e) => handleStatusChange(product, e.target.value.replace('Mark ', ''))}
                        className={`form-select py-1 px-2 text-sm rounded border ${
                          product.status === 'Active' ? 'border-green-300 bg-green-50' :
                          product.status === 'Low Stock' ? 'border-yellow-300 bg-yellow-50' :
                          product.status === 'Out of Stock' ? 'border-red-300 bg-red-50' :
            ...
            <div className="py-3 px-4 flex items-center justify-between border-t border-gray-200">
              <div className="flex items-center">
                <p className="text-sm text-gray-700">
                  Showing <span className="font-medium">{filteredProducts.length === 0 ? 0 : ((currentPage - 1) * PRODUCTS_PER_PAGE + 1)}</span> to <span className="font-medium">{Math.min(currentPage * PRODUCTS_PER_PAGE, filteredProducts.length)}</span> of{" "}
          ...
          <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-md">
            ...
            <div className="mb-6">
              <p className="text-sm text-gray-500">
                Are you sure you want to delete <span className="font-medium text-gray-900">"{deletingProduct.name?.en || 'this product'}"</span>? 
